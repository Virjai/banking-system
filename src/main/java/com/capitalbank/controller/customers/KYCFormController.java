...
public class KYCFormController extends SelectorComposer<Window> {
    ...
    private final KYCDocumentService kycService = new KYCDocumentServiceImpl();   

    /* FORM */
    @Wire private Textbox tFullName, tEmail, tPhoneNo;
    @Wire private Textbox tAadhar, tPanCardNo, tAddress, tCity, tState, tPincode, tCountry;
    @Wire private Datebox dob;
    @Wire private Radiogroup gender;

    /* FILE UPLOADS */
    @Wire private Fileupload aadharUpload, imageUpload;
    @Wire private Button btnSubmit, btnClear;
    ...
    public void submit() {
        ...
        if (dob.getValue() == null) {
            warn("DOB required", dob); return;
        }
        if (gender.getSelectedItem() == null) {
            warn("Select gender", gender); return;
        }
        if (aadharMedia == null || imageMedia == null) {
            warn("Upload both documents", aadharUpload); return;
        }

        String aadhar = tAadhar.getValue();
        if (!aadhar.matches(AADHAR_REGEX)) {
            warn("Invalid Aadhar number", tAadhar); return;
        }

        String pin = tPincode.getValue();
        if (!pin.matches(PINCODE_REGEX)) {
            warn("Invalid Pincode", tPincode); return;
        }
        ...
        try {
            kycService.saveKyc(
                    ...
                    tPincode.getValue(),
                    tCountry.getValue(),
                    aadharMedia.getByteData(),
                    imageMedia.getByteData()
            );
